...
function showWindowHref(){
	...
	var namePosition = urlString.indexOf("name=");
	if(namePosition == -1) url_result.value = "not found.";
	else {
		var nextPositon = urlString.indexOf("&", namePosition);
		if(nextPositon == -1) nextPositon = urlString.length;
		url_result.value = urlString.substring(namePosition + 5, nextPositon);
	}
...
function timeTest(){
	...
	var timeoutID = setInterval( function() {
		number = number * 2;
		timesOfChange = timesOfChange + 1;
		mul.value = number;
		if(timesOfChange == 10) clearTimeout(timeoutID);
	}, 5000);
...
function arrSameStr(){
	var mostString = most.value;
	if(mostString.length == 0) return;
	var charList = {};
	for(var i = 0; i < mostString.length; i++) {
		var char = mostString.charAt(i);
		if(charList[char]) charList[char]++;
		else charList[char] = 1;
	}
	var max = 0; 
	var index;
	for(var i = 0; i < mostString.length; i++) {
		var char = mostString.charAt(i);
		if(max < charList[char]) {
			max = charList[char];
			index = char;
		}
	}
